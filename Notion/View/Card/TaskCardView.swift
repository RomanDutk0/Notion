...
struct TaskCardView: View {
  ...
  var body: some View {
    let filteredTasks = allTasks.filter {
      CardViewModel.status(for: $0, field: selectedViewOption.groupByFieldName!) == cardStatus
    }
    ...
    .onDrop(of: [.text], isTargeted: nil) { providers in
      providers.first?.loadItem(forTypeIdentifier: UTType.text.identifier, options: nil) {
        data, _ in
  ...
  private func moveTask(with id: UUID) {
    if let index = allTasks.firstIndex(where: { $0.id == id }) {
      ...
      if let statusIndex = updatedTask.fieldValues.firstIndex(where: {
        $0.field.name == selectedViewOption.groupByFieldName!
      }) {
...
struct KanbanBoardPreview: View {
  @State private var allTasks: [Task] = [
    Task(fieldValues: [
      ...
      FieldValue(
        field: Field(name: "Status", type: .selection, options: ["To Do", "In Progress", "Done"]),
        value: .selection(["To Do"])),
    ])
  ]
