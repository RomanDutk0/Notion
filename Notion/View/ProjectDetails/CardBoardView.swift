...
struct CardBoard: View {
  ...
  var tasksByStatus: [String: [Task]] {
    Dictionary(
      grouping: tasks,
      by: { CardViewModel.status(for: $0, field: selectedViewOption.groupByFieldName!) })
  }
  ...
  private func bindingForTasks(withStatus status: String) -> Binding<[Task]> {
    Binding(
      get: {
        tasks.filter {
          CardViewModel.status(for: $0, field: selectedViewOption.groupByFieldName!) == status
        }
      },
      set: { newTasks in
  ...
  struct PreviewWrapper: View {
    ...
    static let fields = [
      ...
      Field(
        ...
        options: ["Not Started", "In Progress", "Completed", "Planning"]),
